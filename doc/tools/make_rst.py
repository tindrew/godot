...
redot_DOCS_PATTERN = re.compile(r"^\$DOCS_URL/(.*)\.html(#.*)?$")
...
# `scripts/extract_classes.py` (redotengine/redot-editor-l10n repo) to include them in the source POT file.
BASE_STRINGS = [
    "All classes",
    "Globals",
    "Nodes",
...
def make_rst_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    ...
    with open(
        ...
        source_github_url = f"https://github.com/redotengine/redot/tree/{git_branch}/{source_xml_path}"
        generator_github_url = f"https://github.com/redotengine/redot/tree/{git_branch}/doc/tools/make_rst.py"
...
def make_link(url: str, title: str) -> str:
    match = redot_DOCS_PATTERN.search(url)
    if match:
        groups = match.groups()
