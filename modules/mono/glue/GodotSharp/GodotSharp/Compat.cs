...
{
    /// <inheritdoc cref="SegmentIntersectsConvex(Vector3, Vector3, Redot.Collections.Array{Plane})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public static Vector3[] SegmentIntersectsConvex(Vector3 from, Vector3 to, Redot.Collections.Array planes)
    {
        return SegmentIntersectsConvex(from, to, new Redot.Collections.Array<Plane>(planes));
    }
}
...
{
    /// <inheritdoc cref="AddSurface(Mesh.PrimitiveType, Redot.Collections.Array, Redot.Collections.Array{Redot.Collections.Array}, Redot.Collections.Dictionary, Material, string, ulong)"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public void AddSurface(Mesh.PrimitiveType primitive, Redot.Collections.Array arrays, Redot.Collections.Array<Redot.Collections.Array> blendShapes, Redot.Collections.Dictionary lods, Material material, string name, uint flags)
    {
...
{
    /// <inheritdoc cref="DrawListBegin(Rid, InitialAction, FinalAction, InitialAction, FinalAction, Color[], float, uint, Nullable{Rect2}, Redot.Collections.Array{Rid})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public long DrawListBegin(Rid framebuffer, InitialAction initialColorAction, FinalAction finalColorAction, InitialAction initialDepthAction, FinalAction finalDepthAction, Color[] clearColorValues, float clearDepth, uint clearStencil, Nullable<Rect2> region, Redot.Collections.Array storageTextures)
    {
        return DrawListBegin(framebuffer, initialColorAction, finalColorAction, initialDepthAction, finalDepthAction, clearColorValues, clearDepth, clearStencil, region, new Redot.Collections.Array<Rid>(storageTextures));
    }
}
...
{
    /// <inheritdoc cref="AddTriangleFan(Vector3[], Vector2[], Color[], Vector2[], Vector3[], Redot.Collections.Array{Plane})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public void AddTriangleFan(Vector3[] vertices, Vector2[] uvs, Color[] colors, Vector2[] uv2S, Vector3[] normals, Redot.Collections.Array tangents)
    {
        AddTriangleFan(vertices, uvs, colors, uv2S, normals, new Redot.Collections.Array<Plane>(tangents));
    }
